      ...
      {wishlistProducts.length === 0 ? (
        ...
        wishlistProducts.map((item) => (
          <div key={item._id} style={styles.productItem}>
            <img
              src={UPLOAD_URL + (Array.isArray(item.image) ? item.image[0] : item.image)}
              alt={item.title}
              style={styles.image}
            />
            <div style={styles.details}>
              <h4 style={styles.productTitle}>{item.title}</h4>
              <p style={styles.productPrice}>₹{item.variants?.[0]?.price ?? 'N/A'}</p>
            </div>
            <button onClick={() => handleRemove(item._id)} style={styles.removeBtn}>✖</button>
          </div>
        ))
      )}
    </div>
